#!/usr/bin/env bash
# Copyright (c) 2016 The Genex Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   genexcore.org and genex.org and compares them.
...
WORKINGDIR="/tmp/genex_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://genexcore.org"
HOST2="https://genex.org"
BASEDIR="/bin/"
VERSIONPREFIX="genex-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'genex-',
...
if [ $? -ne 0 ]; then
   echo "genex.org failed to provide signature file, but genexcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "genex.org and genexcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Genex Core binary release signing key installed?"
   fi
