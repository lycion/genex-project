...
The RPM spec file provided here is for Genex-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://genex.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Genex git repository](https://github.com/genex/genex) but are not part of
...
Currently only `Source10` is used, for the example `genex.conf` file.
...
in the genex git repository. At present only an SVG version of the Genex
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the genex
...
needed when 0.12.1 is released, a proper fix is already in the Genex git
master branch.
...
idea to build Genex Core against a static version of that library compiled
...
The `genex.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `genex-server` package. At this point, most RPM based
...
The `genex-server` package should have SELinux support. How to properly do
...
Genex-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/genex/). He has been
packaging Genex for Fedora at least since 2012.
...
4. I renamed the `genex` package that contains the Qt GUI to `genex-core` as
that appears to be how the general population refers to it, in contrast to
`genex-xt` or `genex-classic`. I wanted to make sure the general population
