 ...
 * between genexd, and genex-qt and non-server tools.
*/
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", genex_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/genex/genex>";
    const std::string URL_WEBSITE = "<https://genexcore.org>";
...
/** Sanity checks
 *  Ensure that Genex is running in a usable environment with all
...
{
    // Make sure only a single Genex process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", genex_CONF_FILENAME)).string());
